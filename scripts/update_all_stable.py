...
使用方法:
    nohup python3 scripts/update_all_stable.py > logs/update_stable.log 2>&1 &
"""
import sys
import time
...
logger = setup_logger("update_stable")
...
def stable_update():
    ...
    for i, ts_code in enumerate(stocks, 1):
        ...
        if i % 20 == 0:
            ...
            print(f"[{i}/{total}] {ts_code} - {pct:.1f}%, 剩余{remaining:.0f}分钟")
    ...
    print(f"  失败：{failed} 只")
    print(f"  记录：{rows:,} 条")
    print(f"  耗时：{duration:.0f} 分钟 ({duration/60:.1f} 小时)")
